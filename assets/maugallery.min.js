...
let index=0,next=null;$(imagesCollection).each(function(i){if($(activeImage).attr("src")===$(this).attr("src")){index=i}});next=imagesCollection[index]||imagesCollection[index+1];$(".lightboxImage").attr("src",$(imagesCollection[index+1]).attr("src"))},createLightBox(gallery,lightboxId,navigation){gallery.append(`<div class="modal fade" id="${
        lightboxId ? lightboxId : "galleryLightbox"
            ...
            </div>`)},showItemTags(gallery,position,tags,teste){var tagItems='<li class="nav-item"><span type="button" class="nav-link active active-tag" tabindex="0" data-images-toggle="all">Tous</span></li>';$.each(tags,function(index,value){tagItems+=`<li class="nav-item active">
                <span type="button" class="nav-link" tabindex="0"  data-images-toggle="${value}">${value}</span></li>`});var tagsRow=`<ul class="my-4 tags-bar nav nav-pills">${tagItems}</ul>`;if(position==="bottom"){gallery.append(tagsRow)}else if(position==="top"){gallery.prepend(tagsRow)}else{console.error(`Unknown tags position: ${position}`)}},filterByTag(){if($(this).hasClass("active-tag")){return}
