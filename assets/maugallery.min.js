...
let index=0,next=null;$(imagesCollection).each(function(i){if($(activeImage).attr("src")===$(this).attr("src")){index=i;console.log(imagesCollection[index])}});next=imagesCollection[index]||console.log(imagesCollection[index]);imagesCollection[imagesCollection[index-1]];console.log(imagesCollection[index-1]);$(".lightboxImage").attr("src",$(imagesCollection[index-1]).attr("src"))},nextImage(){let activeImage=null;$("img.gallery-item").each(function(){if($(this).attr("src")===$(".lightboxImage").attr("src")){activeImage=$(this)}});let activeTag=$(".tags-bar button.active-tag").data("images-toggle");let imagesCollection=[];if(activeTag==="all"){$(".item-column").each(function(){if($(this).children("img").length){imagesCollection.push($(this).children("img"))}})}else{$(".item-column").each(function(){if($(this).children("img").data("gallery-tag")===activeTag){imagesCollection.push($(this).children("img"))}})}
let index=0,next=null;$(imagesCollection).each(function(i){if($(activeImage).attr("src")===$(this).attr("src")){index=i}});next=imagesCollection[index]||imagesCollection[index+1];$(".lightboxImage").attr("src",$(imagesCollection[index+1]).attr("src"))},createLightBox(gallery,lightboxId,navigation){gallery.append(`<div class="modal fade" id="${
        lightboxId ? lightboxId : "galleryLightbox"
            ...
            </div>`)},showItemTags(gallery,position,tags,teste){var tagItems='<li class="nav-item"><button type="button" class="nav-link active active-tag" tabindex="0" data-images-toggle="all">Tous</button></li>';$.each(tags,function(index,value){tagItems+=`<li class="nav-item active">
                <button type="button" class="nav-link" tabindex="0"  data-images-toggle="${value}">${value}</button></li>`});var tagsRow=`<ul class="my-4 tags-bar nav nav-pills">${tagItems}</ul>`;if(position==="bottom"){gallery.append(tagsRow)}else if(position==="top"){gallery.prepend(tagsRow)}else{console.error(`Unknown tags position: ${position}`)}},filterByTag(){if($(this).hasClass("active-tag")){return}
